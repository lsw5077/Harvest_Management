...
# Calculate gonadal-somatic index, per Lester et al. 2014 biphasic model
...
makePop <- function(cohortSize, # size of initial cohort
  ...
  startPop <- startPop[rep(seq(nrow(startPop)), as.numeric(startPop$cohortSurvive)),] %>% # expand by cohort number
    ...
    mutate(lAsymGeno = rnorm(1, lAsymMean, lAsymSD), # Draw an asymptotic length
           ...
           #g = findG(k = k), # calculate gonadal-somatic index
           g = g,
           lt = growVB(t = age,
                        k = k,
                        t0 = t0,
    ...
    select(ID, age, lt, lRepro, lAsymGeno, lAsymPheno, g, k, tRepro, mass, stage, vulnerable) %>%
    rowwise() %>%
...
growPop <- function(pop, # population passed from initation function
  ...
  growPop <- pop %>%
    ...
    select(ID, age, lt, lRepro, lAsymGeno, lAsymPheno, g, k, tRepro, mass, stage, vulnerable, legal)
...
reproduce <- function(pop, # population passed from previous step
  ...
  repro_pop <- repro_pop[rep(seq(nrow(repro_pop)), as.numeric(repro_pop$nOffspring)),] %>% # replicate by # of offspring
    ...
    ifelse(is.na(lt), NA, rtruncnorm(n = 1, # Because this is designed for people who think more about fish than evolutionary
           ...
           #g = findG(k), # gonadal-somatic index
           g = g,
  ...
  # Add new juveniles to population  
...
naturalDeath <- function(pop, # population passed
                         ...
                         Nt0,  # pop size at start of sim, passed from function
                         nRecruits, # number of offspring produced, passed in model run
                         compMort) { # whether mortality is compensatory
  
  
 
  # How many offspring were produced divided by number BEFORE HARVEST
  
  r = nRecruits/Nt0
  
  # Make an option for whether harvest mortality is compensatory
  
  dNdt = ifelse(compMort == TRUE,
                r*Nt0*(1-(Nt0 - nHarv)/cc),
                r*Nt0*(1-Nt0/cc))
                

  # Compute new population sizes
  
  target = dNdt + Nt0
  
  # Can't sample more rows than we actually have,
  # so we'll add a correction
  
  target = ifelse(nrow(pop) < target, nrow(pop), target)
  
  # Fish undergo natural mortality
  survive_pop <- pop %>%
    sample_n(round(target)) %>% # sample to density-dependent growing pop
    rowwise() %>%
...
modelSpringSpawn <- function(nSim,
  ...
  for (i in 1:nSim) {
    ...
    OUT$meanG[OUT$sim == i & OUT$t == 1] <- mean(pop$g)
    OUT$sdG[OUT$sim == i & OUT$t == 1] <- sd(pop$g)
    ...
    if (keepPops == TRUE) {write.csv(pop, paste0(popPath, i,"-", j,".csv"))}
      ...
      OUT$meanG[OUT$sim == i & OUT$t == j] <- mean(pop$g)
      OUT$sdG[OUT$sim == i & OUT$t == j] <- sd(pop$g)
